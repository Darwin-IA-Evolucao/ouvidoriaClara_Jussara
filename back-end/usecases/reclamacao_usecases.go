...
func (uc ReclamacaoUseCases) nomePorTelefone(telefone string) string {
	cliente, err := uc.clienteUC.GetClienteByTelefone(telefone)
	if err != nil {
		fmt.Println("[NOME POR TELEFONE] Erro ao buscar cliente: ", err)
		return ""
	}
	return cliente.Nome
}
...
func (uc ReclamacaoUseCases) AprovarInquerito(id string, mensagem string, idUsuario int) error {
	...
	if reclamacao.IDUsuario != nil && *reclamacao.IDUsuario != idUsuario && usuario.Role != "root" {
		return apperror.BadRequest("Você não tem permissão para aprovar esta solicitação")
	}

	if err := uc.repository.UpdateStatusTipo(id, "aprovado", "indicacao", idUsuarioPtr); err != nil {
		return err
	}
	if mensagem != "" {
...
func (uc ReclamacaoUseCases) ColocarEmAnalise(id, data, mensagem string, idUsuario int) error {
	...
	if err := uc.repository.UpdateStatusTipo(id, "em análise", "", idUsuarioPtr); err != nil {
		return err
	}
	if data != "" {
...
func (uc ReclamacaoUseCases) AprovarRequerimento(id string, mensagem string, idUsuario int) error {
	...
	if err := uc.repository.UpdateStatusTipo(id, "aprovado", "requerimento", idUsuarioPtr); err != nil {
		return err
	}
	if err := uc.repository.DesativarAvisos(id); err != nil {
		return apperror.Internal(fmt.Sprintf("Erro ao desativar avisos: %s", err.Error()))
...
func (uc ReclamacaoUseCases) AprovarComoAmbos(id string, mensagem string, idUsuario int) error {
	...
	if err := uc.repository.UpdateStatusTipo(id, "aprovado", "ambos", idUsuarioPtr); err != nil {
		return err
	}
	if err := uc.repository.DesativarAvisos(id); err != nil {
		return apperror.Internal(fmt.Sprintf("Erro ao desativar avisos: %s", err.Error()))
...
func (uc ReclamacaoUseCases) AprovarCausaAnimal(id string, mensagem string, idUsuario int) error {
	...
	if err := uc.repository.UpdateStatusTipo(id, "aprovado", "causa animal", idUsuarioPtr); err != nil {
		return err
	}
	if err := uc.repository.DesativarAvisos(id); err != nil {
		return apperror.Internal(fmt.Sprintf("Erro ao desativar avisos: %s", err.Error()))
...
func (uc ReclamacaoUseCases) FinalizarReclamacao(id string, mensagem string, idUsuario int) error {
	...
	if err := uc.repository.UpdateStatusTipo(id, "finalizado", "", idUsuarioPtr); err != nil {
		return err
	}
	if err := uc.repository.DesativarAvisos(id); err != nil {
		return apperror.Internal(fmt.Sprintf("Erro ao desativar avisos: %s", err.Error()))
	}
	msg := mensagemStatusCliente(uc.nomePorTelefone(reclamacao.Telefone), "finalizada", mensagem)
	err = uc.repository.UpdateMensagem(id, msg)
	if err != nil {
		return err
	}
...
func (uc ReclamacaoUseCases) ReprovarInquerito(id string, mensagem string, idUsuario int) error {
	...
	if err := uc.repository.UpdateStatusTipo(id, "reprovado", "", idUsuarioPtr); err != nil {
		return err
	}
	if err := uc.repository.DesativarAvisos(id); err != nil {
		return apperror.Internal(fmt.Sprintf("Erro ao desativar avisos: %s", err.Error()))
